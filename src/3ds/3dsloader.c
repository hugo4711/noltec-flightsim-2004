...
#include <stdio.h>
//#include <malloc.h>
// FIXED: memory.h einbinden für memset()
//#include <memory.h>
...
static float vector_normalize(vector_t *a,vector_t *b) {
	...
	if(length) {
		// FIXED: konvertierung von double in float
...
static int process_trimesh(FILE *file,unsigned short type,
	...
	if(type == CHUNK_VERTLIST) { /* vertlist */
		...
		for(i = 0; i < trimesh->num_uvmap; i++) {
			...
			// FIXED: Konvertierung in float
			trimesh->uvmap[i].v = 1.0f - read_float(file);
		}
		return 1;
...
//
// Lädt die angegebene Datei und ermittelt die Mesh-Koordinaten.
// num_vertex enthält die Gesamtzahl der verticies des Meshs
// Gibt ein float-array mit den Mesh-Koordinaten zurück
// 
// 
//
float *Load3DS(char *name,int *num_vertex) {
...
// ADDED: Funktionen um das Mesh zu translieren
...
float mesh_radius(float *vertex,int num_vertex) {
    ...
    for(i = 0; i < num_vertex; i++) {
		int j = i * 8;
		tmp = sqrtf(vertex[j + 0] * vertex[j + 0] +
